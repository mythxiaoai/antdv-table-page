...
<template>
  <div class="m-table">
    ...
    <div class="table-operator" style="border-top: 5px">
      <slot name="table-operator" :query="cloneDeep(query)"></slot>
    </div>
    ...
    >
      ...
      <template #_filterDropdown="{ column, confirm }">
        <div class="m-filter" style="padding: 8px">
          <!--component start-->
          <template v-if="checkedComponents.includes(filterFormItem[column.key].component)">
            <component
              :is="filterFormItem[column.key].component"
              v-model:checked="query[column.key]"
              v-bind="filterFormItem[column.key].s"
            ></component>
          </template>
          <!--a-mentions的a-mentions-option处理-->
          <template v-else-if="filterFormItem[column.key].component === 'a-mentions'">
            <component
              :is="filterFormItem[column.key].component"
            ...
            >
              <a-mentions-option
                v-for="sItem of filterFormItem[column.key].s.options"
                :key="sItem.value"
                :value="sItem.value"
              >
                {{ sItem.label }}
...
export default {
  ...
  methods: {
    ...
    initPagination() {
      ...
      //查询
      this.query.current = this.opts.pagination?.current
      this.query.pageSize = this.opts.pagination?.pageSize
    },
    pagingChange({ current }) {
      this.query.current = current
      this.opts.pagination.current = current
      this.list()
    },
    search() {
      this.opts.pagination.current = 1
    ...
    async list() {
      ...
      this.data = data
      if (this.opts.pagination) this.opts.pagination.total = total ?? false
      this.loading = false
      //记录
    ...
    filterReset(column, close) {
      this.query[column.key] = formItemO[column.allkey].value
      close()
    },
    getColor(key) {
    ...
    filterFormItemDeal() {
      ...
      this.opts.columns = columns.map((v) => {
        return dealChaildren(v, (v) => {
          ...
          let key = Object.keys(this.formItem).find((f) => ~f.indexOf(v.dataIndex))
          if (this.formItem[key]?.filter) {
            //先储存全量的key
            v.allkey = key
            //对key :做处理
            if (~key.indexOf(':')) {
...
.m-table .ant-form .ant-form-item {
  display: flex;
  flex-wrap: nowrap;
  margin-bottom: 8px;
}
.m-table .ant-form .ant-form-item .ant-form-item-control-wrapper {
  flex: 1 1 auto;
}
.m-table
  .ant-form
