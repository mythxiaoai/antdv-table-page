<!--  1.2版本支持 全表单过滤器  需要优化:点击过滤 组件控件获取焦点 -->
<template>
  <div class="m-table search-wrapper">
    ...
    >
      ...
      <template #_filterDropdown="{ column, confirm }">
        <div class="m-filter" style="padding: 8px">
          <!-- <a-input
            ...
            @pressEnter="filterSearch(confirm)"
          /> -->
          <!--component start-->
      ...
      <template #_filterIcon="{ column }">
        <FilterFilled
          :style="getColor(column.dataIndex)"
        ></FilterFilled>
      </template>
...
.m-filter .ant-select,.m-filter .ant-slider,.m-filter .ant-time-picker{
  min-width: 180px;
  max-width: 280px;
}
.m-filter .opts {
