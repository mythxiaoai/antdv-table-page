import { watch, computed, unref, onMounted, onUnmounted, ref } from 'vue' //可以导出并设置初始值
import { cloneDeep, isEmpty, isBoolean, isNumber, throttle } from 'lodash'
...
export function useTable(state, props) {
  ...
  let queryComplete = computed(() => {
    ...
    delete res.current
    delete res.pageSize
    res[state.opts.pageKey.current] = current || state.opts.pagination?.current
    res[state.opts.pageKey.pageSize] = pageSize || state.opts.pagination?.pageSize
    let result = {}
    //过滤非空的数据
  ...
  let list = async () => {
    ...
    let data, total
    try {
      ;[data, total] = await state.opts.dataSource(unref(queryComplete))
    } catch (_e) {
      console.error(_e)
    } finally {
      state.loading = false
...
function getRenderFormItem(formItem) {
  ...
  Object.keys(res).forEach((key) => {
    ...
    let edit = o.edit ?? false
      ;['props', 'component', 'value', 'search', 'filter', 'edit'].forEach((v) => {
        delete o[v]
      })
    p = { name: key, ...o }
    //小优化
...
function optimize(component, p, s) {
  ...
  if (placeholderComponent.includes(component)) {
    s.placeholder = s.placeholder ?? (p.label && `请输入${p.label}`)
  }
  //search
